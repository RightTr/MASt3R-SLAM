...
def myprep_for_iter_proj(Xii, Xij, idx_i_to_j_init):
    b, h, w, _ = Xii.shape
    device = Xii.device

    # Ray image
    rays_img = F.normalize(Xii, dim=-1)
    rays_img = rays_img.permute(0, 3, 1, 2)  # (b,c,h,w)
    gx_img, gy_img = img_utils.img_gradient(rays_img)
    rays_with_grad_img = torch.cat((rays_img, gx_img, gy_img), dim=1)
    rays_with_grad_img = rays_with_grad_img.permute(
    ...
    # 3D points to project
    Xij_vec = Xij.view(b, -1, 3)
    pts3d_norm = F.normalize(Xij_vec, dim=-1)

    # Initial guesses of projections
    if idx_i_to_j_init is None:
        # Reset to identity mapping
        idx_i_to_j_init = torch.arange(h * w, device=device)[None, :].repeat(b, 1)
    p_init = lin_to_pixel(idx_i_to_j_init, w)
    p_init = p_init.float()
...
def mymatch_iterative_proj(Xii, Xij, T_CiCj=None, idx_i_to_j_init=None): 
    ...
    rays_with_grad_img, pts3d_norm, p_init = myprep_for_iter_proj(
        Xii, Xij, idx_i_to_j_init
    )
    p1, valid_proj2 = mast3r_slam_backends.iter_proj(
        rays_with_grad_img,
        pts3d_norm,
    ...
    batch_inds = torch.arange(b, device=device)[:, None].repeat(1, h * w)
    dists2 = torch.linalg.norm(
        Xii[batch_inds, p1[..., 1], p1[..., 0], :].reshape(b, h, w, 3) - Xij, dim=-1
    )
    valid_dists2 = (dists2 < cfg["dist_thresh"]).view(b, -1)
    valid_proj2 = valid_proj2 & valid_dists2
