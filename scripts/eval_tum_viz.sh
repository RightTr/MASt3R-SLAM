...
if [ "$print_only" = false ]; then
    for dataset in ${datasets[@]}; do
        ...
        if [ "$no_calib" = true ]; then
            python main.py --dataset $dataset_name --save-as tum/no_calib/$dataset --config config/eval_no_calib.yaml
        else
            python main.py --dataset $dataset_name --save-as tum/calib/$dataset --config config/eval_calib.yaml
        fi
    done
fi
